...
[[nodiscard]] bool StringValidator::isEmpty(const std::string_view str) const {
	return str.empty();
}
...
[[nodiscard]] bool StringValidator::containsAlphaCharacter(const std::string_view str) const {
	const auto isAlpha{ [](unsigned char ch) -> unsigned char { return std::isalpha(ch); } };
	return std::ranges::any_of(str, isAlpha);
}

[[nodiscard]] bool StringValidator::containsDigit(const std::string_view category) const {
	const auto isDigit{ [](unsigned char ch) -> unsigned char { return std::isdigit(ch); } };
	return std::ranges::any_of(category, isDigit);
}

[[nodiscard]] bool StringValidator::containsSpecialCharacter(const std::string_view str) const {
...
[[nodiscard]] bool NameValidator::isValid(const std::string_view name) const
{
	return
...
[[nodiscard]] bool CategoryValidator::isValid(const std::string_view category) const
{
	return
